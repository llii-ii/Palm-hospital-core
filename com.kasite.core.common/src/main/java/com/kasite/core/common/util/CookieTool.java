...
public class CookieTool {
	...
	/**
	 * 添加Cookie,其中Cookie的名称为web名称+键名称
	 * 
	 * @param request
	 *            Request对象
	 * @param response
	...
	public static void addCookie(HttpServletRequest request, HttpServletResponse response, String cookieName,
			String cookieValue, int day) {
		// String webname = request.getContextPath();
		// webname = webname.substring(1);
		// cookieName = webname + "." + cookieName;
		cookieName = "qlc." + cookieName;
		CookieTool.addCookie(response, cookieName, cookieEnCode(cookieValue), day);
	}
		...
		if (cookies != null) {
			for (int i = 0; i < cookies.length; i++) {
				if (cookies[i].getName().equals(cookieName)) {
					/*
					 * cookies[i].setMaxAge(0); cookies[i].setValue(null);
	 ...
	 *            值
	 */
	public static void resetCookieValue(HttpServletRequest request, HttpServletResponse response, String cookieName,
			String cookieValue) {
		Cookie[] c = request.getCookies();
		if (c != null) {
			for (int i = 0; i < c.length; i++) {
				if (c[i].getName().equals(cookieName)) {
					c[i].setValue(cookieEnCode(cookieValue));
	 ...
	 *            天数
	 */
	public static void resetCookieMaxAge(HttpServletRequest request, HttpServletResponse response, String cookieName,
			int day) {
		Cookie[] c = request.getCookies();
		if (c != null) {
			for (int i = 0; i < c.length; i++) {
	...
	public static String getCookieValue(HttpServletRequest request, String cookieName) {
		...
		if (c != null) {
			for (int i = 0; i < c.length; i++) {
				if (c[i].getName().equals(cookieName)) {
					String v = cookieDecCode(c[i].getValue());
					return v;
				}
			}
		}
	...
	/**
	 * 获得某Cookie对应的值,其中Cookie的名称为web名称+属性名称
	 * 
	 * @param request
	 *            Request对象
	 * @param response
