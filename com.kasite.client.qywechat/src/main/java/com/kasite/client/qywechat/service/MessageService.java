...
import com.kasite.client.qywechat.bean.message.resp.RespTextMessage;
import com.kasite.client.qywechat.util.AesException;
import com.kasite.client.qywechat.util.MessageUtil;
...
public class MessageService {
	 ...
	 */
	public String getEncryptRespMessage(HttpServletRequest request, String configKey) {
		...
		try {
			...
			// 1.解密微信发过来的消息-获取明文
			String xmlMsg = this.getDecryptMsg(request, evenToken, encodingAESKey, corpid);
			System.out.println("明文=" + xmlMsg);
			...
			System.out.println("msgType:" + msgType + "  content:" + content);
			System.out.println("Event:" + eventType + "  eventKey:" + eventKey + "  changeType:" + changeType);
			...
			// 6.加密
			WXBizMsgCrypt wxcpt = new WXBizMsgCrypt(evenToken, encodingAESKey, corpid);
	 ...
	 * @return String 消息明文
	 */
	public String getDecryptMsg(HttpServletRequest request, String evenToken, String encodingAESKey, String corpid) {
		// 密文，对应POST请求的数据
		String postData = "";
		// 明文，解密之后的结果
		String result = "";
	...
	public String processEevent(String eventType, String eventKey) {
		...
		// 通讯录事件
		else if (eventType.equals(MessageUtil.EVENT_TYPE_CHANGE_CONTACT)) {
			respContent = "修改通讯录！";
		}
		return respContent;
	}
