...
;(function ($, window, document, undefined) {
	...
	var Tree = function (element, options) {
		...
		return {
			...
			deleteNode: $.proxy(this.deleteNode, this),
   			...
   			//checked Children methods
   			checkedNodeChildren: $.proxy(this.checkedNodeChildren, this),
   			
   			//测试
   			mycheckNode:$.proxy(this.mycheckNode, this)
	 ...
	 *测试 
	 */
	Tree.prototype.mycheckNode = function (identifiers, options) {  
	    alert("进入添加方法1");	    
	    this.forEachIdentifier(identifiers, options, $.proxy(function (node, options) {	    	
	        this.setCheckNode(node, options);
	    }, this));  
	   ...
	   *  测试1 
	   */ 
	Tree.prototype.setCheckNode = function (node, options) { 
		alert(2);
		console.log(options);
		function FindNode(node){ 
			if(node.nodes!=undefined){
				for(var i = 0;i<node.nodes.length;i++){
					...
					//2.修改样式
					//2.1当前选中的node-checked
					//2.2span选中
					//$("#"+node.nodeId+" .glyphicon").removeClass('glyphicon-unchecked').addClass('glyphicon-check');					
					//$("#"+node.nodes[i].nodeId).addClass("node-checked");					
					if(node.nodes[i].nodes!=undefined){
						...
						var obj = node.nodes[i];
						obj.state.checked = true;
//						console.log(options.node);
						node.nodes.push(obj);
//						node.nodes.splice(i,1);a
					}
				}
			}else{
				return null;
	 ...
	 * 自定义选中节点
	 */
	Tree.prototype.checkedNodeChildren = function(node){
	 ...
	 Tree.prototype.setDeleteNode = function (node,  deletenode, options) {
	     ...
	     if(node!=undefined){
	     	...
	     	for(var i = 0; i <this.tree.length; i++){
			       if(this.tree[i].nodeId == deletenode.nodeId){			       
