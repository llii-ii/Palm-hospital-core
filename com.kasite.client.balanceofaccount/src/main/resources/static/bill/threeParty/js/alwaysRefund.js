 ...
 * HIS订单状态查询
 */
function queryHisOrderState(orderId,channelId){
    ...
    jkzl.common.ajaxNoLayer("../../billRefundController/queryHisOrderState.do?v="+(new Date()), param, true, 30000, function (data) {
        myLayer.clear();
        /** 删除弹窗的整个DIV样式*/
        $("#menu").hide();
        /** 调用HIS接口成功，且验证成功，则继续执行调用微信业务*/
...
function oauthCode(orderId, channelId) {
    ...
    jkzl.common.ajaxNoLayer("../../billOauthController/oauthCode.do?v=" + (new Date()), param, false, 30000, function (data) {
        ...
        if ('10000' == data.RespCode) {
            onFlag = true;
            queryHisOrderState(orderId,channelId);
            //dataShowDetail("订单退费",orderId,channelId);
        }else {
            showResult(data.RespMessage);
        }
    });
