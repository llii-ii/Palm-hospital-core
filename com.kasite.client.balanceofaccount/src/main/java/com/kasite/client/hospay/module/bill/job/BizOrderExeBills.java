...
//
///**
// * 轮询三方汇总账单明细数据表中是否存在待执行退费业务的订单(由于冲正业务涉及到与HIS的交互,所有对账平台不处理,只提供数据)
// *
// * @author cc
// */
//@Component
//public class BizOrderExeBills {
//
//     private final static Logger logger = LoggerFactory.getLogger(BizOrderExeBills.class);
...
//
//     /**
//      * 根据业务执行的结果修改三方汇总账单明细数据结果
//      */
////     @Scheduled(cron = "0 0/1 * * * ? ")
//     public void bizChangeEveryDayBalance() {
//          if (flag) {
//               flag = false;
//               try {
...
//                              // 1.更新业务订单执行状态为正在执行
//                              threePartyBalanceDao.updateThreePartyBalanceExeState(Constant.BEINGPAIDSTATE, bill.getOrderId(), bill.getHisOrderId(), bill.getChannelOrderId(), null);
...
//                                   threePartyBalanceDao.updateThreePartyBalanceExeState(Constant.SUCCESSSTATE, bill.getOrderId(), bill.getHisOrderId(), bill.getChannelOrderId(), bill.getAlreadyReceivedMoney());
//
...
//                                   threePartyBalanceDao.updateThreePartyBalanceExeState(Constant.SUCCESSSTATE, bill.getOrderId(), bill.getHisOrderId(), bill.getChannelOrderId(), null);
