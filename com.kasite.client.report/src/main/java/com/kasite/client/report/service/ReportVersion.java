...
public class ReportVersion {
	protected static Logger logger = LoggerFactory.getLogger(ReportVersion.class);
	private static String v = "1.0";
	static {
		logger.info("报告单模块版本号："+v);
		for (ApiMethodName apiModule : ApiModule.Report.values()) {
			logger.info("报告单模块接口api：{}|{}",apiModule.getName(),apiModule.getApiName());
