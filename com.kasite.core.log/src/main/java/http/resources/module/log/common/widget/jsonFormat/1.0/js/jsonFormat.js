window.SINGLE_TAB = "  ";
window.ImgCollapsed = "../widget/jsonFormat/1.0/images/Collapsed.gif";
window.ImgExpanded = "../widget/jsonFormat/1.0/images/Expanded.gif";
...
function CollapseAllClicked(){
  ...
  TraverseChildren($id(jsonWrap), function(element){
...
function CollapseLevel(level){
  ...
  TraverseChildren($id(jsonWrap), function(element, depth){
...
function EnsureIsPopulated(){
  ...
  if(!$id(jsonWrap).innerHTML && !!$id("RawJson").value) Process();
...
function SelectAllClicked(){
  ...
  if(!!range.selectNode)
    ...
    range.selectNode($id(jsonWrap));

  else if(range.moveToElementText)

    range.moveToElementText($id(jsonWrap));

  

  if(!!range.select)

    range.select($id(jsonWrap));
