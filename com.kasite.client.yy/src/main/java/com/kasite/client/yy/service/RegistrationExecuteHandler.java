 ...
 */
@Service
public class RegistrationExecuteHandler implements IBizExecuteHandler {
	...
	public BusinessTypeEnum accept() {
		return BusinessTypeEnum.ORDERTYPE_009;
	}
	...
	public RespPayEndBizOrderExecute bizPayEndExecute(CommonReq<ReqPayEndBizOrderExecute> commonReq) throws Exception {
		...
		if (!KstHosConstant.ONLINEPAY.equals(respOrderDetailLocal.getIsOnLinePay())) {
			LogUtil.info(logger, new LogBody(reqPayEndBizOrderExecute.getAuthInfo()).set("Result",
					"orderid:" + respOrderDetailLocal.getOrderId() + "该预约记录不支持线上支付"));
		...
		if (resp == null) {
			...
			// 返回值为空，未知异常重试
			LogUtil.warn(logger,
					new LogBody(reqPayEndBizOrderExecute.getAuthInfo()).set("挂号订单", respOrderDetailLocal.getOrderId())
							.set("入参", req.getHisReqXml()).set("出参", "返回值为：NULL"));
			respPayEndBizOrderExecute.setBizDealState(BizDealState.BIZ_DEAL_STATE_2);
			return respPayEndBizOrderExecute;
			...
			LogUtil.debug(logger,
					"OrderCallback-orderId:" + respOrderDetailLocal.getOrderId() + ",挂号成功。" + resp.toJSONResult());
			// 挂号成功，返回成功信息
			...
			LogUtil.warn(logger,
					new LogBody(reqPayEndBizOrderExecute.getAuthInfo()).set("挂号订单", respOrderDetailLocal.getOrderId())
							.set("info", "号源锁号信息过期").set("入参", req).set("出参", resp.toResult()));
			respPayEndBizOrderExecute.setBizDealState(BizDealState.BIZ_DEAL_STATE_0);
			return respPayEndBizOrderExecute;
		} else {
			logger.info(MessageFormat.format("订单号: {0} 执行业务异常，返回的结果内容不符合规范。RespCode = {1}", orderId, resp.getCode()));
			// 未知异常，返回失败重试
			LogUtil.warn(logger, new LogBody(reqPayEndBizOrderExecute.getAuthInfo())
					.set("挂号订单", respOrderDetailLocal.getOrderId()).set("入参", req).set("出参", resp.toResult()));
			respPayEndBizOrderExecute.setBizDealState(BizDealState.BIZ_DEAL_STATE_2);
			return respPayEndBizOrderExecute;
		}
	}
...
//		return orderCallBackUtil.callBookStatus(orderView);
		...
		CommonReq<ReqQueryRegInfo> commReq = new CommonReq<ReqQueryRegInfo>(new ReqQueryRegInfo(req.getMsg(), null,
				null, null, req.getParam().getOrderId(), null, null, null, null, null));
