...
public class UpdateByPrimaryKeyWithBLOBsMethodGenerator extends AbstractDAOElementGenerator
{
    ...
    public UpdateByPrimaryKeyWithBLOBsMethodGenerator(final boolean generateForJava5) {
    ...
    public void addImplementationElements(final TopLevelClass topLevelClass) {
        ...
        sb.append("int rows = ");
        sb.append(this.daoTemplate.getUpdateMethod(this.introspectedTable.getIbatis2SqlMapNamespace(), this.introspectedTable.getUpdateByPrimaryKeyWithBLOBsStatementId(), "record"));
        method.addBodyLine(sb.toString());
        method.addBodyLine("return rows;");
        if (this.context.getPlugins().clientUpdateByPrimaryKeyWithBLOBsMethodGenerated(method, topLevelClass, this.introspectedTable)) {
    ...
    private Method getMethodShell(final Set<FullyQualifiedJavaType> importedTypes) {
        ...
        method.setName(this.getDAOMethodNameCalculator().getUpdateByPrimaryKeyWithBLOBsMethodName(this.introspectedTable));
