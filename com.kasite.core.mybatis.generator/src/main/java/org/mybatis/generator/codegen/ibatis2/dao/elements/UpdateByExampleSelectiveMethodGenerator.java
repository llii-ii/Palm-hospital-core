...
public class UpdateByExampleSelectiveMethodGenerator extends AbstractDAOElementGenerator
{
    ...
    public UpdateByExampleSelectiveMethodGenerator(final boolean generateForJava5) {
    ...
    public void addImplementationElements(final TopLevelClass topLevelClass) {
        ...
        sb.append("int rows = ");
        sb.append(this.daoTemplate.getUpdateMethod(this.introspectedTable.getIbatis2SqlMapNamespace(), this.introspectedTable.getUpdateByExampleSelectiveStatementId(), "parms"));
        method.addBodyLine(sb.toString());
        method.addBodyLine("return rows;");
        if (this.context.getPlugins().clientUpdateByExampleSelectiveMethodGenerated(method, topLevelClass, this.introspectedTable)) {
    ...
    private Method getMethodShell(final Set<FullyQualifiedJavaType> importedTypes) {
        ...
        method.setName(this.getDAOMethodNameCalculator().getUpdateByExampleSelectiveMethodName(this.introspectedTable));
