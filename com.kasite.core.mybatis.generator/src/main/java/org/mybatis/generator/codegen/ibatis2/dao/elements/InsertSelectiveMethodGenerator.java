...
public class InsertSelectiveMethodGenerator extends AbstractDAOElementGenerator
{
    ...
    public InsertSelectiveMethodGenerator(final boolean generateForJava5) {
    ...
    public void addImplementationElements(final TopLevelClass topLevelClass) {
        ...
        sb.append(this.daoTemplate.getInsertMethod(this.introspectedTable.getIbatis2SqlMapNamespace(), this.introspectedTable.getInsertSelectiveStatementId(), "record"));
        ...
        if (this.context.getPlugins().clientInsertSelectiveMethodGenerated(method, topLevelClass, this.introspectedTable)) {
    ...
    private Method getMethodShell(final Set<FullyQualifiedJavaType> importedTypes) {
        ...
        method.setName(this.getDAOMethodNameCalculator().getInsertSelectiveMethodName(this.introspectedTable));
