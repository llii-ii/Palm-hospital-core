package org.mybatis.generator.internal.db;

public enum DatabaseDialects
{
    DB2("VALUES IDENTITY_VAL_LOCAL()"), 
    MYSQL("SELECT LAST_INSERT_ID()"), 
    SQLSERVER("SELECT SCOPE_IDENTITY()"), 
    CLOUDSCAPE("VALUES IDENTITY_VAL_LOCAL()"), 
    DERBY("VALUES IDENTITY_VAL_LOCAL()"), 
    HSQLDB("CALL IDENTITY()"), 
    SYBASE("SELECT @@IDENTITY"), 
    DB2_MF("SELECT IDENTITY_VAL_LOCAL() FROM SYSIBM.SYSDUMMY1"), 
    INFORMIX("select dbinfo('sqlca.sqlerrd1') from systables where tabid=1");
    
    private String identityRetrievalStatement;
    
    private DatabaseDialects(final String identityRetrievalStatement) {
        this.identityRetrievalStatement = identityRetrievalStatement;
    }
    
    public String getIdentityRetrievalStatement() {
        return this.identityRetrievalStatement;
    }
    
    public static DatabaseDialects getDatabaseDialect(final String database) {
        DatabaseDialects returnValue = null;
        if ("DB2".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.DB2;
        }
        else if ("MySQL".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.MYSQL;
        }
        else if ("SqlServer".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.SQLSERVER;
        }
        else if ("Cloudscape".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.CLOUDSCAPE;
        }
        else if ("Derby".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.DERBY;
        }
        else if ("HSQLDB".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.HSQLDB;
        }
        else if ("SYBASE".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.SYBASE;
        }
        else if ("DB2_MF".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.DB2_MF;
        }
        else if ("Informix".equalsIgnoreCase(database)) {
            returnValue = DatabaseDialects.INFORMIX;
        }
        return returnValue;
    }
}
