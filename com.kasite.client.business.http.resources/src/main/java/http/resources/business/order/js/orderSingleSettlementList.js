...
function initWidget(){
	...
	var html = '<li class="curr"><a href="javascript:void(0);" value="">全部</a></li>';
	for(var o in isSettlements){
		var v = isSettlements[o];
		html += '<li ><a href="javascript:void(0);" value="'+o+'">'+v+'</a></li>';
	}
	$("#isSettlementUl").html(html);
...
//		page:'orderSingleSettlementList',
//		cardType: Commonjs.constant.cardType_1,
//		id:"filter-jzr",
//		pickerOnClose:function(obj,member){
//			selectedMember = member;
...
//	});
	...
	$('.c-main').on('click','.input-checkbox',function(){
		...
		if(checked){
			$.each( $(this).parent().parent().find('.input-pack'),function(i,o){
				$(o).removeClass('checked').find('input').removeAttr('checked');
			})
			...
			selecedPrescNos = $(this).parent().parent().find("span[data-info='1']").attr("data-presNo");
			selectedCount++;
		}
