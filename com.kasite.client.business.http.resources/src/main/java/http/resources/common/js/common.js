 ...
 */
	Commonjs = {
		...
		getChannelIdName:function(clientId){
			if(clientId == 'minipay'){
				return "Mini付智慧系统";
			}else if(clientId == '100123'){
				return "微信公众号";
			}else if(clientId == '100125'){
				return "支付宝生活号";
				...
				return "医生工作站快捷付";
			}else if(clientId == 'minipay'){
				return "迷你付渠道";
			}else if(clientId == 'smallpro'){
				return "微信小程序渠道";
			}else{
				return "未定义渠道名";
			}
		...
		goToUrl:function(toUrl){
			myLayer.load('正在加载');	
			if(toUrl.indexOf("?UUID=")>0 || toUrl.indexOf("&UUID=")>0){
				return toUrl;
			}
		...
		ajax:function(url,param,successfun,other){
			...
			$.ajax({
	            ...
	            error: function (XHR, textStatus, errorThrown) {
	            	...
	            	option.error(XHR, textStatus, errorThrown);
	            },
	...
	public static final String ORDERTYPE_999 = "999";
		...
		getSession:function(){
			if(window.localStorage){
				...
				if(stoken == token){
		           ...
		           if(jsonObj!=null && jsonObj.RespCode==10000){
		           		return jsonObj;
		           }
				}
			}
		...
		getDiyConfig:function(){
			var hosid = Commonjs.hospitalId();
			//	@PostMapping("/{clientId}/{orderId}/{serviceId}/getConfigKey.do")
			var url = '../../wsgw/'+ hosid +'/getDiyConfig.do';
			var key = "_diyconfig";
			...
			if(null != result && result.RespCode == 6666){
				var data = {}; 
		...
		formatCardNo:function(cardno){
			if(cardno){
				var cardNo = cardno;
				if(cardNo.length>10){
					var a = cardno.substring(0,3);
					var b = cardno.substring(cardno.length-5);
					cardNo = a + "**" + b;
				}
				return cardNo;
			}else{
		...
		memberPicker:function(config){
			...
			this.ajax('/wsgw/basic/QueryMemberCardList/callApi.do?t=' + new Date().getTime(),param,function(jsons){
		            ...
		            if (jsons.RespCode == 10000) {
		                if (Commonjs.ListIsNotNull(jsons.Data)) {
		                	...
		                	Commonjs.BaseForeach(jsons.Data, function (i, item) {
		                		...
		                		var valParam = cardNo+","+item.CardType+","+item.MemberName+","+item.MemberId+","+item.HisMemberId;
		                		if( cardType == 14 ){//住院号
		                			valParam = item.HospitalNo+",14,"+item.MemberName+","+item.MemberId+","+item.HisMemberId;
		                		}
		                		if(def && !hasDefaultMenber){//默认
		                    		...
		                    		if(Commonjs.isEmpty(defaultlValue)){
			                			...
			                			defaultlValue = valParam;
		                			}
		                    		hasDefaultMenber = true;
		                    	}else{
		                    		disObj.push(item.MemberName+"("+ Commonjs.formatCardNo(cardNo) +")");
		                	...
		                	if( !Commonjs.isEmpty(defaultlValue) ){
		                    	...
		                    	$("#"+elementId).parent().find(".c-999").html(titeDesc+"："+Commonjs.formatCardNo(defaultlValue.split(",")[0]));
		                    	$("#"+elementId).val(defaultlValue);
		                	}else{
		                		var d = valObj[0]; 
		                		var name = (d.split(",")[2]);
		                		var cardNo = Commonjs.formatCardNo(valObj[0].split(",")[0]);
		                		$("#"+elementId).parent().find(".c-f15").html(name);
		                    	$("#"+elementId).parent().find(".c-999").html(titeDesc+"："+Commonjs.formatCardNo(cardNo));
		                    	$("#"+elementId).val(valObj[0]);
		                	}
		                	
		                	var istrue = false;
		                	var diyConfig = Commonjs.getDiyConfig();
		        			if(diyConfig && diyConfig.memberPicker){
		        				var memberPicker = diyConfig.memberPicker;
		        				if(page && (memberPicker[page] == true || memberPicker[page] == "true" )){
		        					istrue = true;
		        				}
		        			}
		        			if(!istrue){
		        			...
		        			$("#"+elementId).picker({
		        				...
		        				onClose:function(obj){
		        					...
		        					var cardNo = Commonjs.formatCardNo(memberObj.split(",")[0]);
		        			...
		        			if( typeof  ajaxSuccessFunction === "function" ){
		        				var defaultMember = {};
		        				if( !Commonjs.isEmpty(defaultlValue) ){
		        					defaultMember.memberName = defaultlValue.split(",")[2];
			        				defaultMember.cardNo = defaultlValue.split(",")[0];
			        				defaultMember.cardType = defaultlValue.split(",")[1];
			        				defaultMember.memberId =  defaultlValue.split(",")[3];
			        				defaultMember.hisMemberId =  defaultlValue.split(",")[4];
		        				}else{
		        					defaultMember.memberName = valObj[0].split(",")[2];
		...
		checkDefMember : function(url,cardType,isInputMember){
			...
			if(!isSucc){
    			...
    			if(diyConfig && diyConfig.memberPicker){
    				var memberPicker = diyConfig.memberPicker;
    				for(var o in memberPicker){
    					var v = memberPicker[o];
    					if(url.indexOf(o)>0 && (v == true || v == "true" )){
    						isSucc = true;
    					}
    				}
    			}
		...
		msgDialog :function(msg,btnName,url){
			var phtml = '<div class="addpsonpop">';
			phtml += '<div class="addpson">';
//			phtml += '<i class="sicon icon-anclose"></i>';
			phtml += '<i class="an-jzrtb"></i>';
			phtml += '<div class="h50 c-999 c-f15">'+msg+'</div>';
			if(Commonjs.isEmpty(url)){
				phtml += '<a href="javascript:$(\'.addpsonpop\').remove();" class="btn-anadd mt10">'+btnName+'</a>';
			}else{
				phtml += '<a href="javascript:location.href=Commonjs.goToUrl(\''+url+'\');" class="btn-anadd mt10">'+btnName+'</a>';
			}
			phtml += '</div>';
			phtml += '<div class="addpson-mb"></div>';
			phtml += '</div>';
		...
		/**
		 * 设置页面本地缓存，会话结束或浏览器关闭时自动清除
		 */
		setSessionItem : function(key,value){
			var keyName = window.location.pathname+"_"+key;
			if (window.sessionStorage) {
				window.sessionStorage.setItem(keyName,value);
			}
		},
		/**
		 * 获取页面本地缓存，会话结束或浏览器关闭时自动清除
		 */
		 ...
		 * key		缓存key
		 * value	缓存值
		 * isAddPrefix	key是否添加页面地址前缀，true 时，key=window.location.pathname+"_"+key,这样不同页面使用同一个key进行缓存时不会相互影响
		 */
		setLocalItem : function(key,value,isAddPrefix){
			if(isAddPrefix){
				key = window.location.pathname+"_"+key;
			}
			if (window.localStorage) {
		 ...
		 * 获取页面本地缓存,不清除永久存在
		 * key			缓存key
		 * isRemove		获取后是否删除缓存内容
		 * isAddPrefix	key是否添加页面地址前缀，true 时，key=window.location.pathname+"_"+key,这样不同页面使用同一个key进行缓存时不会相互影响
		 */
		getLocalItem : function(key,isRemove,isAddPrefix){
			if(isAddPrefix){
				key = window.location.pathname+"_"+key;
			}
			var value = "";
		 ...
		 * isAddPrefix key是否添加页面地址前缀，true 时，key=window.location.pathname+"_"+key,这样不同页面使用同一个key进行缓存时不会相互影响
		 */
		removeLocalItem : function(key,isAddPrefix){
			if(isAddPrefix){
				key = window.location.pathname+"_"+key;
			}
			if (window.localStorage) {
