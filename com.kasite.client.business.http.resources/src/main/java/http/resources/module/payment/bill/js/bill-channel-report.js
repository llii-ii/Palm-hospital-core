...
function initData(){
	...
	Commonjs.ajax('/billReport/queryChannelBankCheckList.do',param,function(d){
		var str = "<thead><tr><th>交易场景</th><th>服务类型</th><th>所属银行</th><th>银行账号</th><th>支付方式</th><th>医院应净收</th><th>渠道实净收</th><th>差额(渠道实净收-医院应净收)</th><th>对账结果</th><th>查看</th></tr></thead><tbody>";
		if(d.RespCode == 10000) {
			if(Commonjs.ListIsNull(d.Data)) {
				...
				Commonjs.BaseForeach(d.Data, function(i, item) {
					...
					if(i==0){
						...
						var channelName_ = beforeObj.ChannelName;
					...
					if(i < d.Data.length-1){
						...
						str+="<tr>"
							+"<td><span class=\"c-bold\">"+channelName+"</span></td>"
