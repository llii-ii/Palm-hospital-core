jQuery.yihuUpload = { 
	       ...
	       createUploadForm: function(id, fileElementId, data)  
            {  
                ...
                if (data) {  
                 	...
                 	for(var i in _data){
                  		if (_data[i]&& typeof(data[i]) =='string' && _data[i].indexOf('=')>0) {
	    ...
	    ajaxFileUpload: function(s) {
			...
			var form = $.yihuUpload.createUploadForm(id, s.fileElementId,s.data);
			var io = $.yihuUpload.createUploadIframe(id, s.secureuri);
			var frameId = 'jUploadFrame' + id;
			var formId = 'jUploadForm' + id;		
			...
			{			
				...
				{				
					...
					{
						 xml.responseText = io.contentDocument.body?io.contentDocument.body.innerText:null;
	                	xml.responseXML = io.contentDocument.XMLDocument?io.contentDocument.XMLDocument:io.contentDocument;
					}						
	            }catch(e)
				{
					$.yihuUpload.handleError(s, xml, null, e);
				}
				...
				{				
	                ...
	                try {
	                    ...
	                    if ( status != "error" )
						{
	                        ...
	                        var data = $.yihuUpload.uploadHttpData( xml, s.dataType );    
	                        ...
	                        $.yihuUpload.handleError(s, xml, status);
	                } catch(e) 
					{
	                    status = "error";
	                    $.yihuUpload.handleError(s, xml, status, e);
	                }
