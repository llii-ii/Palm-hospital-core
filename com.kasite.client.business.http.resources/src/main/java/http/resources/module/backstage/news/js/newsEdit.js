...
function _save(dtype,title,smallPic,bigPic,contents,status,link,typeClass){
	...
	var du = Commonjs.ajaxSync('/article/updateArticle.do',param,function(du){
...
//用于兼容对象的foreach方法,第一个参数传入对象，第二个参数传方法体
...
//			Service.Page = page;
			var param = {};
			param.reqParam = Commonjs.getApiReqParams(Service);
			Commonjs.ajaxSync('/article/queryArticle.do',param,function(du){
				if(du.RespCode == 10000){
					BaseForeach(du.Data,function(i, data){
					 smallClick =1;
					 bigClick =1;
					 initUe((data.Contents == undefined || data.Contents == '') ? '' : data.Contents);
					 if(!Commonjs.isEmpty(data.ImgUrl)){
						 var smallImg=data.ImgUrl;
						 smallImg=smallImg.replace("<![CDATA[<",'').replace(">]]","");
						 $("#SmallImgV").attr('src',location.protocol + '//' + location.host + smallImg);
					 }
					 if(!Commonjs.isEmpty(data.BigImgUrl)){
						 $("#BigImgV").attr('src',location.protocol + '//' + location.host + data.BigImgUrl);
					 }
					 $("#dtitle").val(data.Title);
					 $("#dtypeClass").val(data.TypeClass);
					 $("#dtype").val(data.Type);
					 $("#dlink").val(data.LinkUrl);
					 $("#dstatus").val(data.Status);
					 is_head = data.IsHead;
					   if( parseInt(data.FinalDeal) ==  0){
						   $("#ar2").show();//文章内容
							$("#ar1").hide();//链接地址
							$("#dtype").val(0);//文章类型
						}else{
