        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    var url = list[i].url.substring(list[i].url.indexOf(urlPrefix),list[i].url.length);
                    img.setAttribute('src', url + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
