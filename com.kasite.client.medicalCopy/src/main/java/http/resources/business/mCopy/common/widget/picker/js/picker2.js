...
+ function($) {
  ...
  var Picker = function (params) {
      ...
      p.updateValue = function () {
          ...
          if (p.input && p.input.length > 0) {
              $(p.input).val(p.params.formatValue ? p.params.formatValue(p, p.value, p.displayValue) : p.value.join(' 年 '));
  ...
  $.openPicker = function(tpl, className, callback) {
    ...
    var container = $("<div class='weui-picker-container "+ (className || "") + "'><div class='close-picker picker-screen'></div></div>").appendTo(document.body);
    container.show();
