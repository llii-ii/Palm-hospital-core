...
//	                	console.log(ar[i].substring(ar[i].length-1,ar[i].length));
                		if(ar[i].substring(ar[i].length-1,ar[i].length) != '镇'){//过滤掉镇
	            	...
	            	if(v == 2){
	            		//如果第3级没有内容则把第二级选中的值当作第三级的值
	            		var v2 = document.getElementById(s[v-1]);
	            		document.getElementById(s[v]).appendChild(new Option(v2.value, v2.innerHTML));
	            	}
	            }
	        }
	        //二级三级选择
        	if(pks[v]){
        		for(var i = 0 ; i < ss.options.length ; i++){
        			if(ss.options[i].text == pks[v]){
        				ss.options[i].selected = true;
        			}
        		}
	    	}
	        if(System.getComponent('#s'+(v+1))){
	        	System.getComponent('#s'+(v+1)).reload();
	        }
        	if (++v < s.length) {
	        	_this.change(v);
	        }
	...
	var pks = [];
	this.setup = function(p_key){
		p_key = p_key||'';
		//得到p_key数组
		pks = p_key.split(',');
	    for (i = 0; i < s.length - 1; i++){
	    	document.getElementById(s[i]).setAttribute('onchange','geo.change('+(i + 1)+');');
	    }
	    _this.change(0);
	    if(typeof pks[0] != 'undefined'){
	    	preselect(pks[0]);
	    }
	}
	return this;
