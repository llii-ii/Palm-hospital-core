...
(function($,win,undef){
	...
	var tipmsg={//默认提示文字;
		tit:"Tips",
		w:"Please enter the correct information!",
		r:"",
		c:"Detecting information ...",
		s:"Please fill in the information!",
		v:"Filled information is not verified, please wait ...",
		p:"Submitting data ...",
		err:"Wrong! Please check the address or return data submitted in the correct format!",
	...
	Validform.util={
		dataType:{
			...
			//XXX:nullmsg=必填项		errormsg=请输入mm~nn个任意字符，不允许有空格
			"*6-16":/^[\w\W]{6,16}$/,    //任意给定长度的字符 
			
			//XXX:errormsg=请输入mm~nn个任意字符，不允许有空格
			"#*6-16":/(^[\w\W]{6,16})$|(^$)/,  //可为空，或者任意给定长度的英文字符 
			
			//XXX:nullmsg=必填项		errormsg=请输入整数
			"n":/^\d+$/,                 //任意数字  
			
			//XXX:nullmsg=必填项 	errormsg=请输入m~n位整数
			"n6-16":/^\d{6,16}$/,        //任意给定长度的数字 
			"dbFieldProperty6-16":/(^\d{6,16},\d{6,16}$)|(^\d{6,16}$)/,        //任意给定长度的数字 
			
			//XXX:errormsg=请输入m~n位整数
			"#n6-16":/(^\d{6,16}$)|(^$)/,  //可为空，或者任意给定长度的数字
			
			
			//任意字符，可以有空格——————可以有数字、符号、中英文字符，不能含日文韩文等
			//XXX:nullmsg=必填项
			"s":/^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]+$/,  //任意字符 
			
			//XXX:nullmsg=必填项 	 errormsg=请输入mm~nn个任意字符
			"s6-18":/^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]{6,18}$/,  //任意给定长度的字符  
			
			//XXX:errormsg=请输入mm~nn个任意字符
			"#s6-18":/(^[\u4E00-\u9FA5\uf900-\ufa2d\w\.\s]{6,18}$)|(^$)/, //可为空，或者任意给定长度的字符 
			...
			//英文、数字和下划线的组合，不允许有空格
			//XXX:nullmsg=必填项  	 errormsg=请输入mm~nn个字符，可以是英文、数字和下划线的组合，不允许有空格
			"e6-16":/^[a-zA-Z0-9_]{6,16}$/,  
			
			//Add liujia No.8953------------------>
			//英文、数字、减号和下划线的组合，不允许有空格
			"allowminuse6-16":/^[a-zA-Z0-9_-]{6,16}$/,  
			//Add liujia No.8953<------------------
			
			//XXX:errormsg=请输入mm~nn个字符，允许英文、数字和下划线，不允许有空格
			"#e6-16":/^[a-zA-Z0-9_]{6,16}$|(^$)/,  
			"#e":/^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$|(^$)/,
			...
			//XXX:nullmsg=必填项		 errormsg=请输入mm~nn个字符，不允许有空格，不能包含^./_^<>!@#$%&*()-=+|\{}[]'":;?,
			"slimt6-18":/^[^\.\/_\^<>!@#\$%&\*()\-\=\+\|\\\{\}\[\]'":;\?,]{6,18}$/,//任意给定长度的字符(不包括特殊字符) 
			
			//Add liujia No.8953------------------>
			//XX:nullmsg=必填项		 errormsg=请输入mm~nn个字符，允许下划线和减号，不允许有空格，不能包含^./^<>!@#$%&*()=+|\{}[]'":;?,
			"allowminundlimt6-18":/^[^\.\^<>!@#\$%&\*()\=\+\|\\\{\}\[\]'":;\?,]{6,18}$/,
			
			//可以为空	 errormsg=请输入mm~nn个字符，允许下划线和减号，不允许有空格，不能包含^./^<>!@#$%&*()=+|\{}[]'":;?,
			"#allowminundlimt6-18":/(^[^\.\^<>!@#\$%&\*()\=\+\|\\\{\}\[\]'":;\?,]{6,18}$)|(^$)/,
			//Add liujia No.8953<------------------
				
			//XXX:errormsg=请输入mm~nn个字符，不能包含^./_^<>!@#$%&*()-=+|\{}[]'":;\?,
			"#slimt6-18":/(^[^<>]{6,18}$)|(^$)/, //可为空，或者任意给定长度的字符(不包括特殊字符) 
			
			//XXX:errormsg=请输入mm~nn个字符，不允许有空格，不能包含^./_^<>!@#$%&*()-=+|\{}[]'":;?,
			"search-slimt6-18":/^[\w\W]{6,18}$|(^$)/,//任意给定长度的字符
			
			//XXX:nullmsg=必填项		errormsg=IP格式错误
			"ip":/^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])$/, 
			"int": /^\+?[0-9][0-9]{0,4}$/, //非0正整数，且最大为99999
			"int2":/^[0-5]?[0-9]{1}$/ ,//正整数，且不超过59
			"weight":/^[0-9]?\.[1-9]{1}$/ ,//两位小数，小数点后为一位
			"digital":/^(\d{1,3})(\.[0-9]{1,2})?$/,
			"float":/^[0-9]\d*\.?\d*$/
		...
		regcheck:function(datatype,gets,obj){
			//Del liujia No.8544 ---------------------->
			//var nullmsgcontent = obj.attr("nullmsg");
			//if (nullmsgcontent !== null || nullmsgcontent !== undefined || nullmsgcontent !== '') {
			//	obj.attr("nullmsg","必填项");
			//} 
			//Del liujia No.8544 <----------------------
		...
		showmsg:function(msg,type,o,show){//o:{obj:当前对象, type:1=>正在检测 | 2=>通过}, show用来判断tiptype=1的情况下是否弹出信息框;
			...
			if(type==2 && o.obj){
				...
				var next = o.obj.next();
				if(!next.hasClass('Validform_checktip'))
				{
					o.obj.after("<span class='Validform_checktip'>"+msg+"</span>");
				}
				else
				{
					next.text(msg);
		...
		check:function(curform,settings,subpost,bool){
			/*
				检测单个表单元素;
		...
		check:function(bool,selector){
			...
			*/
			...
			curform.find(selector).each(function(){
				Validform.util.check.call(this, curform, obj.settings,"",bool) || (flag=false);
			});
